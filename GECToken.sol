...
contract GECToken is StandardToken, BurnableToken
{   
    string public constant name = "The company green energy";
    ...
    constructor () public
    {
        ...
        uint ownersPart = totalSupply_.mul(20).div(100);
        
        balances[msg.sender] = totalSupply_ - ownersPart;
        emit Transfer(0x0, msg.sender, totalSupply_ - ownersPart);
        
        address owners = 0x4390fbA6D73E8E505bEb2EfDA852304f37000c0d;
...
{   
    ...
    uint256 public constant preIcoStart = 1533081600 - 51 days; // Wed, 01 Aug 2018 00:00:00 GMT
    uint256 public constant preIcoEnd = 1538351999; // Sun, 30 Sep 2018 23:59:59 GMT
    
    uint256 public constant icoStart = 1538352000; // Mon, 01 Oct 2018 00:00:00 GMT
    uint256 public constant icoEnd = 1569887999; // Mon, 30 Sep 2019 23:59:59 GMT


    uint256 public constant preIcoSaleLimit = 500000 * 10 ** 18; // GEC
    uint256 public constant minPurchase = 160 finney; // 0.16 ETH
    
    GECToken public token;
    address public constant wallet = 0x4390fbA6D73E8E505bEb2EfDA852304f37000c0d;
    uint256 public constant price = 575; // GEC per 1 ETH

    uint256 public constant softCap = 34780 ether; 
    ...
    function finish() onlyOwner public
    {
        ...
        {   
            ...
            uint256 fee = balance.div(100).div(5); // 0.2%
    ...
    function getBonus(States state, uint256 tokens, uint256 weiAmount) public constant returns (uint256) 
    {
        uint256 bonus = 0;
        if(state == States.PreICO)
        {
            bonus = tokens.mul(35).div(100);
        }
        else if(state == States.ICO)
        {
            ...
            else if(weiAmount >= 20000 ether)
            {
                bonus = tokens.mul(15).div(100);
            }
            else if(weiAmount >= 50000 ether)
            {
                bonus = tokens.mul(20).div(100);
            }
            else if(weiAmount >= 100000 ether)
            {
                bonus = tokens.mul(25).div(100);
            }
            else if(weiAmount >= 200000 ether)
            {
    ...
    function buyTokens(States state, address to, uint256 weiAmount) internal
    {
        ...
        uint256 bonus = getBonus(state, tokens, weiAmount);
